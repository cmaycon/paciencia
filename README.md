...
*   **UX/UI no Terminal:** Sistema de limpeza de tela dinâmica (compatível com Windows/Linux/Mac) para fixar o tabuleiro, além de **Códigos ANSI** para colorir as cartas e ocultamento de cartas viradas para baixo.
...
*   `estruturas.py`: O núcleo de dados. Contém as classes `Pilha`, `Fila` e `ListaLigada`, além de lógicas avançadas para extrair sublistas e manipular visualmente as cartas ocultas.
...
*   **Lista Ligada 1 a 7:** As colunas da mesa onde o jogo acontece.
    *   **Cores Dinâmicas:** As cartas de Copas (C) e Ouros (O) aparecerão destacadas em vermelho no seu terminal.
    *   **Cartas Ocultas (`[?]`):** Seguindo as regras clássicas, as cartas debaixo das colunas começam viradas para baixo e aparecerão como `[?]`. Você só pode interagir com cartas reveladas. Ao mover a última carta revelada de uma coluna, a próxima carta `[?]` será automaticamente virada para cima.
...
*   **6- de uma Lista Ligada para outra (Mover Sublistas):** Esta é a jogada mais estratégica. O sistema pedirá a Lista de Origem, a Lista de Destino e a **Posição** da carta.
    *   *Nota sobre Posições:* O índice `0` representa a primeira carta revelada do topo da coluna. Você não pode usar como origem uma posição que seja uma carta oculta `[?]`.
